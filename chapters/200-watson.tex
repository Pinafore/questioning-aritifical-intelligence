...
It's been over a decade since \watson{} appeared on \abr{tv}, but the
passage of time has not dulled the luster of this 
revolutionary \textit{tour de force}.  This chapter will talk about how it changed my life, how it
...
company that was synonymous with computing for much of the twentieth century .



It had previously wowed the world by creating ``Deep Blue'', a chess-playing
computer that in 1997 defeated Gary Kasparov, who was widely considered the best chess
player of the time~\citep{hsu-02}.

Google/DeepMind had a similar ambition when they created AlphaGo. In a similar vein to DeepBlue, in 2016 AlphaGo defeated Lee Sedol, who was widely considered the best Go player of the time~\citep{koch-16}.
%
Other \abr{ai} grand challenges include those issued by funding agencies like
the United States' Defense Advance Projects Research Administration
...
big enough splash that people change how they think about technology.

It's hard to imagine an alternate world where these grand challenges did not
happen, but I don't think it's an exaggeration to say that these grand challenges
...
in Chapter~\ref{sec:leaderboards:cooperation}).
%
Likewise, the best Go players became more creative and novel moves by 
incorporating computer-like play into their play style~\citep{shin-25}.
%
...
So what made \jeopardy{} the right pick for this grand challenge?
...
a question, which was done in reaction to infamous human cheating scandals (we discuss how
...
would have the response \answer{Who is Alan Turing}.
%
...
get it right they get that amount added to their score (but a wrong
...
by \abr{ibm} to play trivia games, but who couldn't say anything
more.
...
%
What was I doing wasting my time working on language models~\citep{boyd-graber-10:linguistic} if trivia games were
...
chapter) is the big thing these days, Watson firmly belonged to the
...
Before the statistical age of \abr{qa} was the \emph{rule-based}
age: systems that were impressive on individual questions about
baseball or geology but that failed miserably as soon as it saw a question
that was unexpected in terms of the domain.
...
old-fashioned'' approaches that defined the rule-based systems (Chapter~\ref{sec:ir:gofai-qa}).
%
...
%
Like many of the systems in
this era, it takes an approach that’s somewhat similar to to the old-fashioned AI
approaches: parse the query with a [IY: with a what? something missing here, or it should just be "parse the query"] and look it up in a knowledge base.
%
...
%
Becomes the lookup~\citep{katz-02}
\begin{verbatim}
(get "imdb-movie" "Gone with the Wind (1939)"
                  "DIRECTOR") =>
    ("George Cukor" "Victor Fleming" "Sam Wood"))
\end{verbatim}
%
Some of these can be looked up directly in datasets, but \textsc{start}
further builds on the systems like \textsc{lunar} and
\textsc{baseball} we talked about in previous chapters.
%
While those systems also need to turn the raw text of questions into a structured
representation, \textsc{start} had to do that and deal with the
...
Retrieval~(Chapter~\ref{sec:methods:deep-retrieval}).
...
One way of thinking about question answering is as a translation
...
%
Part of the mythology of \jeopardy{} is that Merv Griffen said in response to said scandal, why
...
into answers. Then, the contestants need to give a response to that answer
...
Ohio, you’ll find distracting things about \abr{us} president Grover Cleveland), etc.
...
original question.
%
Later on, when we talk about dense passage retrieval, those systems
are doing something similar, except they’re generating the query in a
continuous dense vector space
(Chapter~\ref{sec:methods:deep-retrieval}).
%
Doing it with real words like \abr{qasm} is more interpretable: a human could
...
%
The answer is, when a question comes in, \watson{} searches over previous \jeopardy{}
questions, Wikipedia, articles, and a select subset of the Internet to find
evidence that can answer the question (Section~\ref{sec:ir:tf-idf}).
...
%
The guess process looks to see if it has ever seen this exactly clue before; it has, so it produces the guess \answer{waddle}.
...
is larger than 1.0, so it buzzes in with that guess (We'll talk more about how to set these numbers not-so-arbitrarily shortly).
...
% As a reminder Irene thinks this question is bollocks
...
%
But the category has two pieces of information. First, the answer should be ten
letters long; \answer{goalie} fails but \answer{goalkeeper} fits.
%
In addition, the quoted part of the category tells the buzzer that it
...
Let’s go to a question that I got while I was on \jeopardy{}  This demonstrates
that such features aren’t just a category-specific phenomenon, but can appear in individual clues as well.  In a quite normal category about
...
What made \watson{} such a \textit{tour de force} is that humans had to generate each of these features.
%
This generation is not a one and done process. It’s very rare to get the features right the first time around. For instance, maybe the system got
...
case.  
%
Therefoore, if you are building a system to answer such questions, you must look at things the system is still getting wrong and
add a new feature to correct the error.  This is the same process that
Watson used to build its statistical system.  

This statistical approach has some advantages over the neural approach
...
%
For example, suppose that the constraint is that the correct answer starts with a
particular letter.
%
Often, this is straightforward in the category name: \textbf{Starts
  with ``L''}.
%
So you might have a feature that matches that exactly.
%
But then you discover that this feature misses \textbf{Crossword Clues
  ``P''}, and you can check that the feature didn't fire for that
example and generalize that category. [IY comment: I am not sure what this means. How does a start with L clue go off for Crossword Clues P?]

The category that I feared the most was anagrams: where you need to
rearrange the letters in the clue to get to the answer.
%
This is actually easier for a computer than a human!
%
If you have ``anagram'' in the category name, you create a feature
that is true if the set of letters matches for both the clue and the guess.
...
the category.  Not so!
%
Sometimes the pattern reveals itself as you work your way through a
category.  For example \textbf{Let's Make a Movie Crossover!} requires
you to combine two movie titles (or characters) together.
%
So you would want a feature that would check for consistency with
previous correct responses in the category.

And for ``normal'' clues, generating a guess looks a lot like the
reformulations that we saw in QASM: [IY note: this is not in this chapter or the intro, make sure it's ok here]

\question{In May 1898 Portugal celebrated the 400th anniversary of
  this explorer's arrival in India}
A 400\textsuperscript{th} anniversary of 1898 gets rewritten to May 1498, India gets transformed into Kappad, and then you can find something with the correct response: \answer{Vasco de Gama}.
...
to solve anagrams, others look for rhymes, others run an \abr{ir} query like the
Start system we talked about before, others are doing transformations like the
\abr{qasm} approach we talked about before.
%
And sometimes the approaches need to take a recursive approach, combining
different subsystems to get to the right answer.

\subsection{Deciding When to Trust a Guess: Which Features Matter?}
\label{sec:watson:logistic-regression}

From the above approaches, you generate dozens of possible guesses.  How do you know which---if
any—to select?
%
Combining inputs to predict a value is often called a regression
problem, and when the prediction is a binary decision, this is called
a \emph{logistic regression} problem.
% This all gets fed into a logistic regression problem.
This
...
%
This will require a bit of math, so if you just want to jump ahead to
the big picture,\footnote{This will be our first ``technical''
  section.  You don't need to read the technical sections to
  understand the rest of the non-technical parts of the book, but
  subsequent technical sections \emph{will} depend on previous
  technical sections.} go ahead to
Section~\ref{sec:watson:feature-engineering}.
...
%
And this should look familiar: it looks a lot like the dot product that we
first saw in the chapter on information retrieval
\ifproposal
(Chapter~\ref{ch:ir}),
\else
(Equation~\ref{eq:ir:dot-product} in Chapter~\ref{ch:ir}),
\fi
where we compute the \emph{similarity} of
two examples by multiplying the \tfidf{} scores of two documents' words.

And this is a good segue into \emph{why} we write this as a dot product rather
...
answer.
%
And the weights will implicitly encode how important a feature is for
making a decision.
...
probability~$\pi_i$ between zero and one that gets big when the weights pass the threshold~$b$:
%
\begin{equation}
  \pi_i = \sigma \left( \sum_j w_{j} x_{i,j} - b \right) \equiv p(\mbox{buzz} \g {\bm x}) \equiv \frac{1}{1+\expp{-\left(\sum_{j} w_j
        x_{i,j} + b \right)}}.
  \label{eq:watson:pi}
\end{equation}
%
...
\begin{equation}
  \mathcal{L} = -\sum_{i \in \mathcal{X}_c}
    \log{\pi_i} - \sum_{i \in
    \mathcal{X}_f} \log{1 - \pi_i}.
...
then Equation~\ref{eq:watson:loss} flips it around so that the larger it is,
the wronger we are; we want to minimize it.
...
buzzed---$y_i=1.0$---or you shouldn't have---$y_i=0.0$.
%
However, the
...
The other input to the update is how much of this feature the example has:~$x_{i,j}$.
%
...
that Watson famously got wrong as a final \jeopardy{} when it answered with the
great American city \answer{Toronto}.
%
So in this case our buzzer input\footnote{The calculus for Final
  \jeopardy{} is different: you always provide a guess, so there's no buzzing.  However, you still want
  the best answer to have the highest probability, so we can still work
  through this example.  Given the debugging output \watson{}
wasn't  confident, so it probably
  wouldn't have ``buzzed in'' during normal play.} is whether we should buzz on this clue
...
\begin{tabular}{ccc}
  ...
  \midrule
  \abr{ir} Score & 2 & 0.1 \\
  \underline{Toronto} compatibility with Category & 1 & 0.2 \\
...
%
the category feature encodes whether the answer is compatible with the
...
\end{align}
This shows the dangers of having unbalanced feature weights and overly large step
...
%
In a presentation, \abr{ibm} showed that Watson got that clue
...
Something unique to \jeopardy{} is that the category (which is prominently displayed at the top of each column of clues) is a huge
constraint on what the correct responses are.
%
A large part of 
\jeopardy{} gameplay is figuring out the ``lexical answer type'': what kind of
thing can the answer be and only buzzing in on the things that are consistent
with that type.\footnote{The Watson project called these signifiers
  lexical answer types, but the Quizbowl community
  (Section~\ref{sec:manchester:qb}) uses the term ``pronoun'' or
  ``identifier''.  I prefer lexical answer type, however, so I'll
  stick with that.}
%
Part of
this is is looking at sub-clues within the clue, such as ``this woman'' (Sue
...
can learn that all of the answers should be a month. [IY: what does getting a couple of months wrong mean?]
%
While we don't know details of this implementation, we can
...
%
Resources like WordNet~\citep{miller-90} can help with this: it
provides a tree-like organization for all of the nouns in English so
that you can check whether ``June'' is a month, whether a
``schnauzer'' is a dog, or whether ``pride'' is an emotion.
%
...
One thing that made \watson{} particularly groundbreaking was that it not only
computed raw accuracy, but also compared itself against human performance.
%
This chart shows how Watson progressed in different iterations of the system,
inching up to the cloud of \jeopardy{} champions.  Ken Jennings is in red there,
still clearly dominating even the final version of Watson. [IY comment: I don't see this chart]
%
...
%
To understand why, it's useful to go over the lifecycle of an entire question: how it was
...
not necessarily challenge computers. [IY comment: these seem like the same problem to me]
...
difficult\footnote{The difficulty ratchet isn't one way; all ``special'' matches use designated questions: ``Celebrity
  ...
  Questions are tuned to expected players' abilities.  \abr{ibm} did not want the computer to be targeted,
  lest the questions be adversarial against the computer.
  Chapter~\ref{sec:leaderboards:adversarial} discusses what this would look like if we
...
internalize the cadence of Alex's voice and when a tech would
...
many identical questions~\citep{lewis-21}, which makes it an imperfect yardstick. [IY comment: I don't understand what this is conveying in the context of this book]
%
...
wordplay~\citep{kaushik-18}. [IY: I deleted the next 3 paragraphs bc I think this one made the point, and I was not sure what they were saying]

In other words, the ``difficulty'' of \jeopardy{}
questions on the board were calibrated for humans; they do not apply to computers.
%
We talk about how Facebook/Meta dealt with this problem in
Chapter~\ref{sec:leaderboards:adversarial}: tell the authors what's hard for a computer.
...
%
You might think this is the scenario: a question comes in, and Watson has the choice of answering it or not:
...
point out that this isn't the case, because the clues are not weighted
...
make them unhappy with you, because they can make your life miserable.
%
...
\end{quote}
%
And nobody, nobody, wanted to make Alex unhappy.

Ken would sometimes do a little hunting for the Daily Double against a
particularly formidable opponent, but he would normally be
well-behaved so as not to upset the powers that be.
%
\watson{}, however, was a soulless machine; and having a machine on the
stage was so exciting that
nobody faulted its strategy.
%
...
producers (or worse, Alex Trebek), that code would be deleted in no
time.
...
%
We've seen claims that computers are smoking lawyers in the
\abr{lsat}~\citep{malik-24} or are better than radiologists in reading X-rays~\citep{bellamy-22}.
%
...
how to actually measure how far \abr{ai} has come since Ken Jennings lost to
