...
\end{equation}
In other words, if you got this example right, you slightly---how much
is controlled by the step size parameter~$\lambda$---increase all of
...
in Table~\ref{table:watson:features}.\footnote{Lest there be any doubt, I am making up these weights and
  ...
  features.  And you normally don't have feature weights with such
  round numbers, but it makes the example cleaner.}
...
From Equation~\ref{eq:watson:update}, the error is 0.4---$\pi_i$
should have been zero, but it was 0.4---and if we assume\footnote{In
  practice, this would be a really bad step size---it's way too
  large---but we'll go with this because it makes the math easier.} that our step
size is 1.0.
%
