...
\section{Cleverdon's Clever Collection}
\label{sec:ir:cranfield}
...
were roughly comparable~\citep{cleverdon-67}.
...
\section{Is it Better to Find All the Good Things or for Everything Found to be Good?}
\label{sec:ir:precision-recall}
...
Sometimes you want to balance precision and recall.  A common
combination of these two measures of goodness is $F_1$ score, which is
the harmonic mean of precision and recall:
\begin{equation}
  F_1 \equiv = 2 \frac{\mbox{precision} \cdot
    \mbox{recall}}{\mbox{precision} + \mbox{recall}} = 
\label{eq:f-score}
\end{equation}

So, if you have a set of queries $q_1$\dots $q_N$ you can run your
system on each question, get a set of results, compute the precision
and recall of each one, and then create a ranking of the systems.

Let’s pause for a moment to recognize how revolutionary this was.
%
Once you found the relevant documents, you can evaluate \emph{any}
system.
%
Before, you had to run an expensive user experiment every time you
tweaked your system: did that output look good?
%
How about this one?
%
With a reusable test collection, you could turn the crank on your
retrieval system without pesky humans getting in the way.  Just check
whether the red box finds the relevant documents!
...
\subsection{Which Words Matter Most in your Index?}
\label{sec:ir:tf-idf}

We will talk about neural representations later when we talk about
models like \abr{bert} (Section~\ref{sec:methods:deep-retrieval}).
%
...
%
In this case, that would be ``Lincoln''\dots but another seemingly
important is ``the'' since it also appears just as often.

The great intuition of Sp\"ark Jones is that while you and I know that
``the'' is unimportant, and you could make a list of such unimportant
words (often called a ``stop'' list in the lingo), that list is going
to be brittle.
%
For example, ``resolved'' is an unimportant word in legislative
contexts.
%
The thing that ties together unimportant words is that they appear in
many different questions.

Mathematically, you want to downweight words that appear in many
documents.  Sp\"ark Jones motivated inverse document frequency as
\begin{quote}
[placing] greater emphasis on the value of a term as a means
of distinguishing one document from another than on its value as an indication of the content of the document itself
\end{quote}
%
In other words, you want to combine the term frequency (this term is
important to the document) with the
inverse document frequency (this word is important overall).
%
The most common way to do this~\citep[there are countless combinations with
funny names like BM25]{robertson-09}; the details are important in
practice but not for understanding the theory.
%
The most common way to do this is to weight a term $w$ in document $d$
as number of times $w$ is in document $d$ relative to the total number
of terms in $d$, and inversely by the number of documents the word $w$
appears in $N_d$.
%
The term frequency is
\begin{equation}
\mathrm{tf}(w,d) = \frac{n(w,d)}{\sum_{w' \in d} n(w',d)},
\end{equation}
where the denominator sums over all of the other words in the
document, and the inverse document frequency is
\begin{equation}
\mathrm{idf}(w) = \log \frac{N}{N_d},
\end{equation}
where we use $\log$ to not so carefully distinguish very frequent
words (``the'') from fairly frequent words (``very''); both are about
as unimportant.

But this only gives the \emph{representation} of a document.
%
How does this go from a query to finding relevant answers?
%
Given a query representation $\vec q$---a vector with a sparse entry
for every term---and a document representation $\vec d$---also a
vector with a sparse entry for every term---to find the similarity, we
see how similar they are.
%
The more words that overlap, the higher the similarity.
%
Moreover, the higher the weight of the words, the more important their
contribution to the similarity.
%
To turn these intuitions into mathematics, we multiply each of the
terms' weights and add them all up:
\begin{equation}
  \mbox{similarity}(\vec q, \vec d) = \sum_t \vec q_t \cdot \vec d_t.
  \label{eq:ir:dot-product}
\end{equation}
This operation is called a dot product.
%
Because most terms' weight will be zero, this is essentially looking
for overlap of words that are important to the document but that don't
appear in many different documents.
...
area after Karen Sp\"ark Jones.

\section{Old-Fashioned \abr{ai}'s Search for Meaning}
...
%
And this defined the evaluation of how many computers answered questions in the mid twentieth
century.\footnote{This is sometimes called ``good ol' fashioned \abr{ai}'' or
  \abr{gofai} for short.}
%
But the approaches were not just using the term weightings above to
find relevant documents: could we not just find documents to help a
reader answer a question but generate the answer given a question.
%
These approaches went deeper and often used represenations that required
logic and reasoning to answer questions.

Arguably, the history of computer question answering begins with
...
answering in the mid-twentieth century~\citep{sedman-11}:
\begin{quote}
...
Let’s take a look at the \abr{baseball} system
from \abr{mit}.
%
Despite the odd phrasing, the \abr{baseball} system
of Green et al. is essentially a natural language
interface to an existing database~\citep{green-61}.
...
things like ``This Argentenian Author'' could be Borges, Cortazar, or
Puig).
%
For baseball, the scope is relatively narrow: a team, a location, or a
number (number of runs or wins).

Like the search over large document collections the \abr{baseball}
system specifies what it is looking for using what it calls a
specification list.
%
If the quesiton is \question{Where did the Red Sox play on July 7?}
...
Month = July
  Day = 7 
\end{verbatim}
and then it finds all of the corresponding records where the Red Sox
played a game on July 7 and uses that to construct an answer.

But if you know a database language like \abr{sql},
...
For example, the question \question{How many samples are there?} is analyzed
...
\end{verbatim}
%
Unlike \abr{baseball} it can handle previous contexts (like
conversational \abr{qa}, Chapter~\ref{sec:formats:conversation}).
...
an \abr{ir} component, although it’s explicitly
%
...
The last old school \abr{qa} system I wanted to
talk about is probably the most famous of
%
them all: \abr{shrdlu}.
%
\abr{shrdlu} was described in the thesis of Terry
Winograd\dots remember that name, we’ll be
%
seeing it again in Chapter~\ref{sec:leaderboards:adversarial}!

Unlike the \abr{lunar} and \abr{baseball} which are exactly
...
First, since you’re reading a book printed\footnote{In all likelihood,
you're not actually reading a printed book, but a boy can dream.} in the twenty-first century,
...
of the information being fed into it (Figure~\ref{fig:ir:linotype}).

\begin{figure}[!ht]
    \subfloat[Linotype Machine\label{fig:ir:linotype-machine}]{%
      \includegraphics[width=0.45\textwidth]{figures/external/wikipedia_commons_Linotype_Simplex_1895}
    }
    \hfill
    \subfloat[Linotype Science Fiction\label{fig:ir:etaoin-shrdlu}]{%
...
%
Despite its funny name, \abr{shrdlu} remains the
\abr{og} question answering system.
...
one you meant (which is still a challenge we discuss in Chapter~\ref{ch:bad}).
%
I’m not going to go through the components
of \abr{shrdlu}, but it is fairly similar to \abr{lunar}:
%
you parse sentences into a logical form and find what matches in the
knowledge base.
%
...
\begin{figure}
  \includegraphics[width=1.0\linewidth]{figures/external/shrdlu}
  \caption{A screen shot of the \abr{shrdlu} system.  Unlike other
    early \abr{qa} systems with a static world, you can move the
    blocks around and ask questions about them.  However, it's a very
    small world.}
\end{figure}

The reason that I'm not talking about the details is because you’ve
probably noticed that these systems are talking about narrower and
narrower
%
domains.
%
...
much)\dots \abr{shrdlu} goes further to build
%
its own world and only answering questions
about that.

And that goes back to the origin of the name
\abr{shrdlu}: taking text and making it into a reality
%
...
it.
%
That is what all of the systems in this chapter would have done: it's
hard coded into their interpretations.
%
But natural language is funny and ambiguous
sometimes.

And that’s the big difference between modern
\abr{qa} systems and these older systems.
%
...
\section{Making Hide and Seek (in Documents) a Professional Game}
\label{sec:ir:nist}

And that's exactly the long-lasting legacy of the Cranfield paradigm shaped
 how we get answers from the internet.
 %
 For that, we need to leave central England and have the mantle get
 taken up by a new player: \abr{nist}.  The
...
insight with Cyril Cleverdon combined with the vision of Karen Sp\"ark Jones,
the story of \abr{nist}’s development of these test collections is one of slow,
...
that by the time Google’s PageRank~\citep{brin-98} came around, Google
used the same evaluations internally to optimize their algorithms.

Just fire up the latest test collection, compute your precision and recall, and call it a day.
%
And this approach toward evaluation is with us in everything we do.  Test
collections are how we evaluate virtual assistants, new \abr{ai}
systems, and just about every aspect of the modern Internet (Chapter~\ref{ch:leaderboards}).  And it’s not
...
unlike people who work on, say, human-computer interaction, those building \abr{qa}
...
\jbgcomment{Would be nice to have clearer connection to Google}