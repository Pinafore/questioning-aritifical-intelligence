...
%
But the history is much deeper than that; there are spies,
misrepresentations, deceptions, and attempting to unmask pretenders.
%
This chapter shows how this history of deception defined how we
approach \abr{ai} today.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=.8\linewidth]{figures/external/Alan_Turing_18-10-2009_11-10-27~~}
    \end{center}
...
Bletchley Park team) had cracked the code, the British fabricated
alternate explanations of how they gathered valuable intelligence.
%
At one level they attributed the intelligence to a British-run spy
ring in German called `Boniface', and then was called
`Ultra'~\citep{gilbert-15}.
% Footnote 1653 
%
For example the British engineered a flyover by a
reconnaissance plane to ``spot'' a German ship as an alternate
explanation for how the British knew to attack them~\citep{wilcox-01}.
...
Turing's work, Turing himself had to deceive the British government
about his own life.
%
...
%
This is analogous to current \abr{ai} craze: they have read all of the
Internet, and the challenge is to measure whether all of that ingested
knowledge can fuel an incipient intelligence.
...
%
Because if you picked something that correlates with intelligence (say
...
To recap, to have a Turing test, you need \textbf{A Skilled
  Interrogator}, \textbf{An Unrestricted Domain}, and \textbf{Knowledge
...
(and arguably no purpose at all), as Turing’s syllogism fails. It is questionable
...
but also puts in disfluencies such as adding ``um'' and pauses to make
it sound more human.
%
...
%
Chinese text comes in, and he needs to provide a Russian translation.
%
He understands neither language, but can look up the appropriate
...
There are several counterarguments to this criticism: you're not
...
Sheiber argues that the number of sentences in any language is so
...
in the Omahan's lookup to deceive a skilled inquisitor.

\begin{table}[t]
  \centering
  \rowcolors{2}{gray!10}{white}
\begin{tabular}{p{0.3cm}p{3.05cm}p{3.05cm}p{3.05cm}}
\toprule
& 
...
\rotlabel{Interrogator} &
Identify which hidden participant is really the woman, based only on written replies. &
Identify which hidden participant is really the woman, based only on written replies. &
Determine which hidden participant is human and which is a machine, based only on conversation. \\[0.8em]
...
Respond naturally as a human, making it possible for the interrogator to recognize them as human. \\[0.8em]

\rotlabel{Candidate} &
A man impersonates the woman well enough that the interrogator cannot reliably distinguish it from the human impersonator. &
A man \textbf{or a computer} impersonates the woman well enough that the interrogator cannot reliably distinguish it from the human impersonator. &
...
\caption{Different versions of the imitation game and the Turing Test,
  distinguished by participant roles and goals.  While the original
  Imitation Game was about a man impersonating a woman, the Second
  Turing Test is conventionally what people mean when people talk
  about \emph{The} Turing Test.  However, I agree with
  \citet{Sterrett-00}'s take that the \textbf{First Turing Test} is
  implied by Turing's proposal, and in many ways make more sense.}
...
of his eponymous test (Table~\ref{table:turing:tests}).\footnote{This is an argument because Turing's
  ...
  was inadvertent or intentional.  I prefer to think of it as
  intentional, but it fits with this chapter's theme of connecting
...
\citet{Sterrett-00} says that a dullard in the Second Turing test must do nothing more
...
must create a fictitious persona that will be probed by the
interogator and also craft responses in a way that will elicit the
...
as a human.
%
\citet{hendricksen-24} argues that children learning to lie is a
...
the Times obituary of the game's creator, Allan Calhamer~\citep{fox-13}:
\begin{quote}
...
\begin{figure}
  ...
  \caption{Although Meta claimed that human players did not recognize
    when they were playing against computer opponents (an unannounced,
    imprompteau Turing Test), \citet{Wongkamjan-24} show that players
    in a game of \textit{Diplomacy} \emph{can} reliably detect an
    \abr{ai} opponent.  $F_1$ Score is a combination of precision and
    recall, as introduced in Section~\ref{sec:ir:precision-recall} (Equation~\ref{eq:f-score}).}
\end{figure}

And although few people are running Turing Test experiments
today,\footnote{And even those who are running them are doing it as a
  one-off; as far as I know, no major company has a Turing Test
  division.} it
...
%
It also helps it easier to understand why asking the right questions (Chapter~\ref{ch:bad})
in the right way are so important: we won't have a valid evaluation (Chapter~\ref{ch:leaderboards}) if
